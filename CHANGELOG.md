...
### 新增 (Features)
- **行业对比雷达**
  - 将原来的「财报异常雷达」升级为「行业对比雷达」，与亮点/风险形成互补：文字摘要（上）+ 定量对比（下）
  - 后端 `BuildRiskRadar` 增加行业均值对比，调用 `GetIndustryMetrics` 获取行业基准数据
...
- **财报异常雷达**
  ...
  - 覆盖维度：应收账款异常、存货周转下降、现金流质量、ROE 水平、负债水平、A-Score 风险、非财务风险

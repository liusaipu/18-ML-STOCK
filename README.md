...
- **可比公司分析**：添加 3~5 家可比公司，自动计算均值/最高/最低/排名百分位，支持多年度趋势对比；模块4.2采用 A-Score 替代 M-Score，增加活跃度指标，并基于 7 维度加权评分（ROE 25%、毛利率 20%、营收增长 15%、现金含量 10%、负债率 10%、A-Score 10%、活跃度 10%）进行综合排序，给出蓝色亮条投资建议
...
- **智能财报问答**：基于 LLM 的财报解读对话功能。当前评估其实用性有限，暂时不纳入开发计划，后续视需求再议。

#### 未来计划

- **iOS App（SwiftUI）**：基于现有 Go 后端构建移动端体检卡片流，需要作为独立项目启动。
